...
router.post("/", normalizeNullableInt("id_venta"), normalizeNullableInt("id_libro"), validateFK("id_venta", "ventas", "id_venta"), validateFK("id_libro", "libros", "id_libros"), 
async (req, res) => {
    try {
        ...
        const [result] = await db.execute(`INSERT INTO detalle_ventas (id_venta, id_libro, cantidad, precio_unitario) VALUES (?,?,?,?)`, [id_venta, id_libro, can, precio]);
...
router.put("/", normalizeNullableInt("id_venta"), normalizeNullableInt("id_libro"), validateFK("id_venta", "ventas", "id_venta"), validateFK("id_libro", "libros", "id_libros"), 
async (req, res) => {
    try {
        const id = Number(req.params.id);
        if (!Number.isInteger(id)) return res.status(400).json({ error: "ID inválido" });
        ...
        const [result] = await db.execute(`UPDATE detalle_ventas SET id_venta=?, id_libro=?, cantidad=?, precio_unitario=? WHERE id_detalle=?`, [id_venta, id_libro, can, precio, id]);
